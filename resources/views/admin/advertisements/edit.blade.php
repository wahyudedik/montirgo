<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-3">
            ...
            <h2 class="text-xl font-semibold text-gray-800">Edit Iklan: {{ $advertisement->title }}</h2>
        </div>
    </x-slot>
        ...
        <div class="max-w-2xl">
            <form action="{{ route('admin.advertisements.update', $advertisement) }}" method="POST" enctype="multipart/form-data"
                ...
                <div>
                    ...
                    <x-text-input id="title" name="title" :value="old('title', $advertisement->title)" class="mt-1 block w-full" required />
                ...
                <div>
                    ...
                    <img src="{{ asset('storage/' . $advertisement->image_path) }}" alt="{{ $advertisement->title }}"
                        class="mt-2 w-48 h-28 object-cover rounded-lg border border-gray-200">
                </div>

                {{-- New Image --}}
                <div>
                    <x-input-label for="image" value="Ganti Banner (opsional, JPG/PNG/WebP, maks 5MB)" />
                ...
                <div>
                    ...
                    <x-text-input id="target_url" name="target_url" :value="old('target_url', $advertisement->target_url)" type="url" class="mt-1 block w-full" placeholder="https://..." />
                ...
                <div>
                    ...
                    <select id="position" name="position" class="mt-1 block w-full border-gray-300 rounded-lg shadow-sm focus:border-primary focus:ring-primary text-sm" required>
                        @foreach(['banner' => 'Banner (Atas)', 'sidebar' => 'Sidebar', 'feed' => 'Feed', 'popup' => 'Popup'] as $val => $label)
                            <option value="{{ $val }}" {{ old('position', $advertisement->position) === $val ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <x-text-input id="start_date" name="start_date" :value="old('start_date', $advertisement->start_date->format('Y-m-d'))" type="date" class="mt-1 block w-full" required />
                        <x-input-error :messages="$errors->get('start_date')" class="mt-2" />
                    </div>
                    <div>
                        <x-input-label for="end_date" value="Tanggal Selesai" />
                        <x-text-input id="end_date" name="end_date" :value="old('end_date', $advertisement->end_date->format('Y-m-d'))" type="date" class="mt-1 block w-full" required />
                ...
                <div class="flex items-center gap-2">
                    <input type="checkbox" id="is_active" name="is_active" value="1" {{ old('is_active', $advertisement->is_active) ? 'checked' : '' }}
                ...
                <div class="flex justify-end gap-3 pt-4 border-t border-gray-100">
                    ...
                    <button type="submit" class="px-4 py-2 bg-primary hover:bg-primary-600 text-white rounded-lg text-sm font-medium">Perbarui Iklan</button>
                </div>
            </form>
        </div>
    </div>
