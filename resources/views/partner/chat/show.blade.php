    ...
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <div class="flex items-center">
                <a href="{{ route('partner.chat.index') }}" class="text-gray-400 hover:text-gray-600 mr-3">
                    ← Kembali
                </a>
                <div>
                    <h2 class="font-semibold text-lg text-gray-800 leading-tight flex items-center gap-2">
    ...
    <script>
        function chatApp(chatId, orderId, currentUserId) {
            return {
                ...
                async loadMessages() {
                    try {
                        const res = await fetch(`/partner/orders/${orderId}/chat/poll?last_id=0`);
                ...
                async pollMessages() {
                    try {
                        const res = await fetch(`/partner/orders/${orderId}/chat/poll?last_id=${this.lastMessageId}`);
                        const data = await res.json();
                ...
                async sendMessage() {
                    ...
                    try {
                        const res = await fetch(`/partner/orders/${orderId}/chat/send`, {
                            method: 'POST',
                            headers: {
